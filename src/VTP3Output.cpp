 ...
 * @return
 */
std::shared_ptr<VTP3::EthernetHeader> VTP3::ethernet_header(std::string const& if_name, uint8_t dst_mac[MAC_LENGTH], const int sockfd)
{
    ifreq ifr;
    ...
    memcpy(&header->src_mac, &ifr.ifr_hwaddr.sa_data, sizeof(ifr.ifr_hwaddr.sa_data));
...
int VTP3::send(Connection const& connection, const void *data, size_t size)
{
    ...
    memset(&ifr, 0, sizeof(ifreq));
    strncpy(ifr.ifr_name, connection.if_name.c_str(), IFNAMSIZ - 1);
    ...
    memcpy(&eh->dst_mac, &connection.ethernet_header->dst_mac, sizeof(connection.ethernet_header->dst_mac));
    memcpy(&eh->src_mac, &connection.ethernet_header->src_mac, sizeof(connection.ethernet_header->src_mac));

    // Use 802.3 Ethernet frame without Vlan or Ethernet II with Vlan
    eh->type = has_vlan ? htons(ETH_P_8021Q) : htons(data_size - eth_size); /* set length of 802.3 payload */
