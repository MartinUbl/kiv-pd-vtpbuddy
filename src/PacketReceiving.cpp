...
namespace VTP3
{
    ...
    void got_packet(const u_char *packet)
    {
        ...
        if (*((uint16_t *)(packet + SNAP_8023_OFFSET)) == SNAP_DSAP_SSAP_HEADER)
        {
        ...
        else if(*((uint16_t *)(packet + SNAP_ETHERNETII_OFFSET)) == SNAP_DSAP_SSAP_HEADER)
        {
            ...
            return; // not SNAP frame (maybe not even VTP packet) - drop

        //frame-len is on previous 2 bytes
        frame_len = htons(*((uint16_t *)(packet + offset - 2)));
        offset += VTP_VERSION_OFFSET + VTP_MESSAGE_OFFSET;
        vtp_msg_code = *((uint8_t *)(packet + offset));
    ...
    void *pkt_receiving(struct sniffing_data *sd)
    {
        ...
        while(loop_running)
        {
            data_size = recvfrom(sockfd, (char*)buf, BUF_MAX_LEN, 0, &saddr, &saddr_size);
