...
int SubsetAdvertPacket::send(Connection const& connection, std::vector<std::shared_ptr<VlanInfo>> vlans)
{
    ...
    memcpy(&buf[0], &header, sizeof(this->header));
    SubsetAdvertPacket* pkt = reinterpret_cast<SubsetAdvertPacket*>(&buf[0]);
    ...
    for(auto vlan : vlans)
    {
        ...
        memset(&buf[offset], 0, vlan->name_size());
        memcpy(&buf[offset], vlan->name.get(), vlan->data.name_length);

        offset += vlan->name_size();
    }

    return VTP3::send(connection, &buf[0], data_size);
}
